...
const Signup = () => {
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/auth/verify-otp`, { otp }, {
        headers: {
  ...
  const handleProfileUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE}/users/upload-profile`, formDataUpload, {
        headers: {
  ...
  const handleSkipProfile = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/users/skip-profile`, {}, {
      ...
      setError(err.response?.data?.error || 'Skip failed');
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-lg shadow-lg border border-blue-100">
        <div className="text-center">
          ...
          <p className="mt-2 text-navyBlueMedium font-sans" style={{ color: '#2A3A6F' }}>
            {step === 1 ? 'Create your account' : step === 2 ? 'Verify your email with OTP' : 'Add your profile picture'}
          </p>
        </div>
        {error && (
