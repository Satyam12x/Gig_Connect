...
import { Star, Search, Code, PenTool, BookOpen, Users, Briefcase, CheckCircle, Quote, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <div className="flex flex-col md:flex-row gap-12 items-center">
          <div className="flex justify-center md:justify-end order-first md:order-last">
            <div className="relative">
              <div
                className="absolute inset-0 bg-gradient-to-br from-navyBlue via-purple-600 to-blue-800 rounded-full opacity-20 blur-md"
              ></div>
              <div
          ...
          <div className="space-y-8 text-center md:text-left">
            <div className="space-y-4">
              ...
              <p className="text-lg sm:text-xl text-navyBlueMedium max-w-2xl mx-auto md:mx-0 font-sans" style={{ color: '#2A3A6F' }}>
                Discover skilled designers, developers, and tutors right in your campus community. Hire services or showcase your expertise.
              </p>
            </div>
...
const FeaturedGigsSection = () => {
  ...
  useEffect(() => {
    const fetchGigs = async () => {
      try {
        const response = await axios.get(`${API_BASE}/gigs/recent`);
        setGigs(response.data.slice(0, 4)); // Limit to 4 featured gigs
        setLoading(false);
      } catch (err) {
...
const RecentGigsSection = () => {
  ...
  useEffect(() => {
    const fetchGigs = async () => {
      try {
        const response = await axios.get(`${API_BASE}/gigs/recent`);
        setGigs(response.data.slice(0, 6)); // Limit to 6 recent gigs
        setLoading(false);
      } catch (err) {
          ...
          <>
            <div className="flex overflow-x-auto gap-8 pb-4">
              {gigs.map((gig) => (
                <div
                  key={gig._id}
                  className="min-w-[280px] bg-white bg-opacity-70 backdrop-blur-lg p-6 rounded-2xl shadow-lg border border-blue-100 hover:bg-blue-50 hover:border-navyBlueLight transition-colors duration-300 text-center"
                  style={{ clipPath: 'polygon(20% 0%, 80% 0%, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0% 80%, 0% 20%)' }}
                >
                  {gig.thumbnail ? (
                    <img
    ...
    <div className="relative bg-white py-16 sm:py-24 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="mt-4 text-lg text-navyBlueMedium max-w-3xl mx-auto font-sans" style={{ color: '#2A3A6F' }}>
            Find the perfect service for your needs from a variety of student talent categories.
          </p>
        </div>
          ...
          {categories.map((category, index) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-medium mt-2 text-center font-sans" style={{ color: '#1A2A4F' }}>
                  {category.name}
                </span>
              </Link>
            );
  ...
  const steps = [
    {
      icon: Search,
      title: 'Browse Gigs',
      description: 'Explore a variety of services offered by talented students in your campus community.',
    },
    {
      icon: Briefcase,
      title: 'Hire Talent',
      description: 'Connect with skilled students and start your project with confidence.',
    },
    {
      icon: CheckCircle,
      title: 'Complete Project',
      description: 'Get your project delivered on time through our secure platform.',
    },
  ];
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div
                ...
                style={{ clipPath: 'polygon(20% 0%, 80% 0%, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0% 80%, 0% 20%)' }}
              >
                <Icon size={32} className="text-navyBlueLight mx-auto mb-4" style={{ color: '#3A4A7F' }} />
                <h3 className="text-xl font-semibold text-navyBlue mb-2 font-sans" style={{ color: '#1A2A4F' }}>
                  {step.title}
                </h3>
                <p className="text-navyBlueMedium font-sans" style={{ color: '#2A3A6F' }}>
          ...
          <div className="flex justify-center mt-6 space-x-2">
            {testimonials.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full ${index === currentIndex ? 'bg-navyBlue' : 'bg-gray-300'} hover:bg-navyBlueLight transition-colors duration-300`}
  ...
  return (
    <div className="relative bg-navyBlue py-16 sm:py-24 overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h2 className="text-3xl sm:text-4xl font-bold text-white font-sans">Ready to Start?</h2>
        <p className="mt-4 text-lg text-blue-200 max-w-3xl mx-auto font-sans">
          Join Gig Connect today and start finding or posting gigs in your campus community!
        </p>
